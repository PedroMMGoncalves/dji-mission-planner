...
- **Modo circular, circlegrammetry** (`src/utils/circular.js`): a área coberta por uma grelha de círculos sobrepostos, cada um voado com a câmara inclinada (−45° por omissão) e o rumo ao seu centro, uma foto por ponto. A sobreposição entre círculos dá o passo entre centros `2R(1−p)`; a grelha é centrada na caixa da área, alinhada com a aresta mais longa, e sai da fronteira como no UgCS; as fiadas alternam o sentido de rotação e voam-se em serpentina, cada círculo a entrar pelo ponto virado ao anterior. Seguimento de terreno por ponto, blocos por bateria com círculos inteiros, conselho de sobreposição (a maior que não acrescenta um círculo) e o tempo da grelha da Área lado a lado. Geometria de Bilodeau et al. (2025).
...
- **Órbitas multi-nível** (`src/utils/orbit.js`): círculos empilhados em torno de um POI, pontos por volta a partir da sobreposição, rumo ao alvo, gimbal por nível apontado à cota do centro, exportação em voo curvo contínuo — missão única ou um KMZ por nível. Captura em **fotografia** (anéis, uma foto por waypoint) ou **vídeo** (espiral contínua que sobe um passo por volta, a gravar do primeiro ao último ponto).
...
| WPML (KMZ) — Órbita | Voo curvo contínuo, rumo ao POI, gimbal por nível, `_orbit_nN` (única ou ZIP por nível); espiral em vídeo com `startRecord`/`stopRecord`, `_orbit-video_nN` | Inspecção/3D de alvos isolados |
| WPML (KMZ) — Circular | Grelha de círculos sobrepostos, rumo a cada centro, gimbal oblíquo fixo, uma foto por ponto, voo curvo contínuo, `_circular_nN` (única ou ZIP por bloco, `-tf` com relevo) | 3D de vegetação, afloramentos, pedreiras, sítios apertados |
