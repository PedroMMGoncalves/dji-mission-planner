...
- **Circular mode, circlegrammetry** (`src/utils/circular.js`): the area covered by a grid of overlapping circles, each flown with the camera tilted (−45° by default) and heading at its centre, one photo per point. Circle overlap sets the centre spacing `2R(1−p)`; the grid is centred on the area's box, aligned with its longest edge, and overshoots the boundary as UgCS does; rows alternate rotation and are flown as a serpentine, each circle entered at the point facing the previous one. Per-point terrain following, whole-circle battery blocks, an overlap advisor (the largest overlap that does not add a circle) and the Area-grid time side by side. Geometry from Bilodeau et al. (2025).
...
- **Multi-level orbits** (`src/utils/orbit.js`): stacked circles around a POI, points per revolution from the overlap, heading at the target, per-level gimbal aimed at the target centre height, continuous curved flight export — single mission or one KMZ per level. Capture is **photo** (rings, one photo per waypoint) or **video** (a continuous spiral climbing one step per turn, recording from the first to the last point).
...
| WPML (KMZ) — Circular | Grid of overlapping circles, heading at each centre, fixed oblique gimbal, one photo per point, continuous curved flight, `_circular_nN` (single or per-block ZIP, `-tf` with terrain) | 3D of vegetation, outcrops, quarries, confined sites |
| WPML (KMZ) — Face | Fixed heading and one photo per waypoint, `_face_p1-N` | Faces, slopes, structures |
| WPML (KMZ) — Orbit | Continuous curved flight, heading at the POI, per-level gimbal, `_orbit_nN` (single or per-level ZIP); video spiral with `startRecord`/`stopRecord`, `_orbit-video_nN` | Inspection/3D of isolated targets |
...
Every release is archived on Zenodo with its own DOI. To cite a specific
version use its DOI (v1.0.1: 10.5281/zenodo.22238441); to cite the software
regardless of version use the concept DOI **10.5281/zenodo.22238440**, which always
resolves to the latest release. The repository also ships a `CITATION.cff`
