...

{
  ...
  check('KML: anel aberto com 4 vertices (vertice de fecho removido)',
    r.ring.length === 4 && !r.needsCrs, `${r.ring.length} vertices`)
  check('KML: coordenadas na ordem [lon, lat], altitude descartada',
  ...
  check('KML: tags com prefixo de namespace (kml:Polygon) sao lidas',
    rp.ring.length === 4, `${rp.ring.length} vertices`)
  ...
  check('KML: com varios poligonos escolhe o de maior area',
    Math.abs(rBoth.ring[0][0] - -8.5) < 1e-9, JSON.stringify(rBoth.ring[0]))
  ...
  check('KML: poligono degenerado -> erro legivel',
    (await throwsWith(() => parseAreaFile(
      fakeFile('a.kml', kmlDoc(polygonKml('-8.5,39.5 -8.4,39.5')))),
      'Nenhum poligono'.replace('poligono', 'polígono'))) === null)

  check('KML: XML malformado -> "KML invalido"',
    (await throwsWith(() => parseAreaFile(fakeFile('a.kml', '<kml><Document></kml>')),
      'KML inválido')) === null)
  check('KML: sem poligonos -> erro legivel',
    (await throwsWith(() => parseAreaFile(fakeFile('a.kml', kmlDoc('<Placemark/>'))),
      'Nenhum polígono encontrado no KML')) === null)
}

{
  // GeoJSON em WGS84
  const ring = [[-8.5, 39.5], [-8.49, 39.5], [-8.49, 39.51], [-8.5, 39.51], [-8.5, 39.5]]
  const fc = { type: 'FeatureCollection', features: [
    { type: 'Feature', geometry: { type: 'Polygon', coordinates: [ring] } }] }
  const r = await parseAreaFile(fakeFile('a.geojson', JSON.stringify(fc)))
  check('GeoJSON: FeatureCollection -> anel aberto sem needsCrs',
    r.ring.length === 4 && !r.needsCrs, `${r.ring.length} vertices`)

  const multi = { type: 'MultiPolygon', coordinates: [[ring], [ring.map(([x, y]) => [x + 1, y])]] }
  check('GeoJSON: MultiPolygon recolhe todos os aneis exteriores',
  ...
  check('GeoJSON: GeometryCollection e percorrida',
  ...
  check('GeoJSON: reprojectRing devolve o anel original em WGS84',
    back.every((p, i) => Math.abs(p[0] - ring[i][0]) < 1e-7 && Math.abs(p[1] - ring[i][1]) < 1e-7),
    JSON.stringify(back[0].map((v) => +v.toFixed(6))))

  check('GeoJSON: JSON invalido -> erro legivel',
    (await throwsWith(() => parseAreaFile(fakeFile('a.geojson', '{nao json')),
      'GeoJSON inválido')) === null)
  check('GeoJSON: sem poligonos -> erro legivel',
    (await throwsWith(() => parseAreaFile(fakeFile('a.geojson', '{"type":"Point","coordinates":[0,0]}')),
      'Nenhum polígono encontrado no GeoJSON')) === null)
  check('extensao nao suportada -> erro legivel',
    (await throwsWith(() => parseAreaFile(fakeFile('a.dxf', 'x')),
      'Formato não suportado')) === null)
}

{
  ...
  check('simplifyRingIfNeeded: > 400 vertices reduzido mantendo a forma',
    simplified.length <= 400 && simplified.length >= 3, `${many.length} -> ${simplified.length}`)
  const areaBefore = turf.area(turf.polygon([[...many, many[0]]]))
  const areaAfter = turf.area(turf.polygon([[...simplified, simplified[0]]]))
  check('simplifyRingIfNeeded: area preservada a menos de 1%',
  ...
  globalThis.DOMParser = class {
    parseFromString() {
      return new real().parseFromString(
        '<parsererror xmlns="http://www.w3.org/1999/xhtml">erro</parsererror>', 'application/xml')
    }
  }
...

{
  ...
  const wps = [[-8.500, 39.500], [-8.495, 39.500], [-8.495, 39.504], [-8.500, 39.504]]
  ...
  check('WPML ida e volta: numero de waypoints',
    back.waypointCount === 4 && back.waypoints.length === 4, `${back.waypointCount}`)
  check('WPML ida e volta: coordenadas preservadas (6 casas decimais)',
    back.waypoints.every((p, i) =>
      Math.abs(p[0] - wps[i][0]) < 1e-6 && Math.abs(p[1] - wps[i][1]) < 1e-6),
    JSON.stringify(back.waypoints[0]))
  check('WPML ida e volta: altitude, velocidade e nome',
    back.altitude === 100 && back.speed === 7 && back.name === 'missao-teste',
    `${back.altitude} m, ${back.speed} m/s, "${back.name}"`)
  check('WPML ida e volta: enums de aeronave e payload',
  ...
  check('WPML: anel reconstruido e um poligono valido',
  ...
  check('WPML: todos os waypoints caem dentro do anel reconstruido',
...

{
  ...
  const wps = [[-8.5, 39.5, 90], [-8.495, 39.5, 100], [-8.495, 39.504, 110], [-8.5, 39.504, 130]]
  const kmz = await makeKmz({ waylines: buildWaylinesWPML(missionParams(wps)) })
  const back = await parseWpmlKmz(kmz)
  check('WPML: alturas diferentes -> mediana das alturas',
    back.altitude === 105, `${back.altitude} m`)

  const flat = wps.map(([x, y]) => [x, y, 80])
  const backFlat = await parseWpmlKmz(await makeKmz({ waylines: buildWaylinesWPML(missionParams(flat)) }))
  check('WPML: alturas iguais -> altura unica, nao mediana', backFlat.altitude === 80, `${backFlat.altitude} m`)
}

{
  ...
  const shuffled = doc(pm(2, -8.49, 39.51) + pm(0, -8.50, 39.50) + pm(1, -8.49, 39.50))
  const back = await parseWpmlKmz(await makeKmz({ waylines: shuffled }))
  check('WPML: ordem de voo vem do <wpml:index>, nao da ordem no ficheiro',
  ...
  check('WPML: sem <wpml:index> segue a ordem do documento',
...

{
  ...
  const back = await parseWpmlKmz(await makeKmz({ waylines: buildWaylinesWPML(missionParams(line)) }))
  check('WPML: waypoints colineares -> rectangulo de recurso com 4 vertices',
    back.ring.length === 4, `${back.ring.length} vertices`)
  const poly = turf.polygon([[...back.ring, back.ring[0]]])
  check('WPML: rectangulo de recurso contem a linha com folga',
    back.waypoints.every((p) => turf.booleanPointInPolygon(turf.point(p), poly)))
  const north = Math.max(...back.ring.map((p) => p[1]))
  check('WPML: folga do rectangulo ~20 m',
    Math.abs((north - 39.5) * 110574 - 20) < 1, `${((north - 39.5) * 110574).toFixed(1)} m`)
}

{
  // o template serve de reserva quando o waylines nao tem rota
  const wps = [[-8.5, 39.5], [-8.49, 39.5], [-8.49, 39.51]]
  const back = await parseWpmlKmz(await makeKmz({ template: buildTemplateKML(missionParams(wps)) }))
  check('WPML: so com template.kml a rota e lida na mesma',
    back.waypointCount === 3 && back.altitude === 100, `${back.waypointCount} wp, ${back.altitude} m`)
...
{
  check('WPML: zip corrompido -> erro legivel',
    (await throwsWith(() => parseWpmlKmz(new Uint8Array([1, 2, 3, 4])),
      'KMZ ilegível')) === null)
  check('WPML: zip sem waylines nem template -> erro legivel',
  ...
  check('WPML: missao sem coordenadas -> erro legivel',
    (await throwsWith(() => parseWpmlKmz(makeKmz({
      waylines: '<?xml version="1.0"?><kml><Document><Folder/></Document></kml>',
    })), 'nenhum waypoint encontrado')) === null)
  check('WPML: XML corrompido dentro do zip -> erro legivel',
    (await throwsWith(() => parseWpmlKmz(makeKmz({ waylines: '<kml><Document></kml>' })),
      'XML da missão ilegível')) === null)
  check('WPML: poucos waypoints para um anel -> erro legivel',
    (await throwsWith(() => parseWpmlKmz(makeKmz({
      waylines: buildWaylinesWPML(missionParams([[-8.5, 39.5], [-8.49, 39.5]])),
    })), 'waypoints suficientes')) === null)
}
 ...
 */
const TIFF_TYPE = { ASCII: 2, SHORT: 3, LONG: 4, DOUBLE: 12 }
const GEO_KEY_ID = { GTModelTypeGeoKey: 1024, GeographicTypeGeoKey: 2048, ProjectedCSTypeGeoKey: 3072 }

function makeFloatTiff({ width, height, valueAt, originX, originY, scale, geoKeys, nodata, transform = null }) {
...

{
  ...
  check('MDT CRS: projectado suportado (PT-TM06)',
  ...
  check('MDT CRS: geografico tratado como lon/lat directo',
    geo.geographic === true && geo.def === null && geo.crsCode === 'EPSG:4326')
  check('MDT CRS: ETRS89 (4258) tambem conta como geografico',
    resolveRasterCrs({ GeographicTypeGeoKey: 4258 }).geographic === true)
  check('MDT CRS: projectado tem precedencia sobre o geografico',
    resolveRasterCrs({ ProjectedCSTypeGeoKey: 3763, GeographicTypeGeoKey: 4326 }).crsCode === 'EPSG:3763')
  check('MDT CRS: user-defined (32767) conta como ausente',
    (await throwsWith(async () => resolveRasterCrs({ ProjectedCSTypeGeoKey: 32767 }),
      'não indica o sistema de coordenadas')) === null)
  check('MDT CRS: sem GeoKeys -> erro que diz o que exportar',
    (await throwsWith(async () => resolveRasterCrs({}), 'PT-TM06 (EPSG:3763)')) === null)
  check('MDT CRS: EPSG desconhecido -> erro que nomeia o codigo',
    (await throwsWith(async () => resolveRasterCrs({ ProjectedCSTypeGeoKey: 32633 }),
      'CRS não suportado: EPSG:32633')) === null)
}

{
  // projectBox: identidade, curvatura das arestas e falha total
  const box = projectBox([0, 0, 10, 20], (a, b) => [a, b])
  check('projectBox: identidade devolve a mesma caixa', box.join(',') === '0,0,10,20', box.join(','))
  ...
  check('projectBox: amostra as arestas, nao so os cantos',
    Math.abs(curved[3] - 9) < 1e-9, `maxY ${curved[3].toFixed(2)} (cantos dariam 5)`)
  check('projectBox: todos os pontos fora do dominio -> erro legivel',
    (await throwsWith(async () => projectBox([0, 0, 1, 1], () => { throw new Error('fora') }),
      'Não foi possível projetar a área')) === null)
}

{
  ...
  check('MDT: CRS e dimensoes da janela lida',
    out.crsCode === 'EPSG:3763' && out.width === 40 && out.height === 30,
    `${out.crsCode} ${out.width}x${out.height}`)
  check('MDT: resolucao nativa e a do raster (2 m)',
  ...
  check('MDT: elevacao no centro do pixel e o valor do pixel',
  ...
  check('MDT: interpolacao bilinear entre pixeis vizinhos',
  ...
  check('MDT: coordenadas nao finitas devolvem null',
...

{
  ...
  check('MDT: zona de nodata devolve null (nao o sentinela -9999)',
  ...
  check('MDT: zona valida ao lado do nodata mantem-se correcta',
  ...
  check('MDT: janela toda sem dados -> erro legivel',
    (await throwsWith(() => loadDemFromFile(allVoid, bboxFrom(-30000, 20000, -29920, 19940), { marginM: 0 }),
      'não tem valores de elevação válidos')) === null)

  // sentinela enorme de float (−3.4e38) tambem conta como sem dados
  const huge = makeDem({ valueAt: (x) => (x >= 20 ? -3.4e38 : 50), nodata: -32768 })
  const outHuge = await loadDemFromFile(huge, bboxFrom(-30000, 20000, -29920, 19940), { marginM: 0 })
  check('MDT: sentinela -3.4e38 tratado como sem dados',
...

{
  ...
  const out = await loadDemFromFile(dem, bboxFrom(-30000, 20000, -29920, 19940), { marginM: 0, maxDim: 10 })
  check('MDT: maxDim limita o lado maior da grelha lida',
    out.width === 10 && out.height <= 10, `${out.width}x${out.height}`)
  check('MDT: resolucao efectiva cresce com a reamostragem',
  ...
  const fine = await loadDemFromFile(makeDem(), bboxFrom(-30000, 20000, -29920, 19940), { marginM: 0 })
  const p = toWgs(-30000 + 40, 20000 - 30)
  const coarseV = out.elevationAt(...p)
  const fineV = fine.elevationAt(...p)
  check('MDT: elevacao reamostrada concorda com a nativa no mesmo ponto',
    Math.abs(coarseV - fineV) < 5, `${coarseV?.toFixed(1)} vs ${fineV?.toFixed(1)} m`)
}

{
  // raster em graus (EPSG:4326): sem reprojeccao
  const dem = makeDem({
    originX: -8.5, originY: 39.51, scale: 0.0002, width: 20, height: 20,
  ...
  check('MDT: resolucao de um raster em graus convertida para metros',
...
{
  // erros de entrada
  check('MDT: ficheiro invalido -> erro legivel',
    (await throwsWith(() => loadDemFromFile(null, [0, 0, 1, 1]), 'Ficheiro de MDT inválido')) === null)
  check('MDT: bbox invalida -> erro legivel',
    (await throwsWith(() => loadDemFromFile(makeDem(), [0, 0, NaN, 1]),
      'Área de levantamento inválida')) === null)
  check('MDT: ficheiro que nao e GeoTIFF -> erro legivel',
    (await throwsWith(() => loadDemFromFile(new Blob([new Uint8Array(64)]), [-8.5, 39.5, -8.49, 39.51]),
      'Não foi possível abrir o GeoTIFF')) === null)
  // raster com rotação: recusado em vez de georreferenciado a torto
  const rotated = makeFloatTiff({
    width: 20, height: 20, valueAt: () => 50, scale: 2, originX: -30000, originY: 20000,
    geoKeys: { GTModelTypeGeoKey: 1, ProjectedCSTypeGeoKey: 3763 }, nodata: -9999,
    // [a, b, c, d, e, f, ...]: b e e não nulos = eixos rodados
    transform: [2, 0.5, 0, -30000, 0.5, -2, 0, 20000, 0, 0, 0, 0, 0, 0, 0, 1],
  })
  check('MDT: raster com rotacao -> recusado com instrucao de reexportar',
    (await throwsWith(() => loadDemFromFile(rotated, bboxFrom(-30000, 20000, -29960, 19960), { marginM: 0 }),
      'MDT com rotação não suportado')) === null)
  check('MDT: raster que nao cobre a area -> erro legivel',
    (await throwsWith(() => loadDemFromFile(makeDem(), bboxFrom(200000, 200000, 200100, 200100),
      { marginM: 0 }), 'não cobre a área de levantamento')) === null)
}
