...
function titleCasePt(s) {
  return s
    ...
    .split(/\s+/)
    .map((w, i) =>
      i > 0 && MINOR.has(w) ? w : w.charAt(0).toLocaleUpperCase('pt') + w.slice(1),
    )
    .join(' ')
}
...
for (const f of raw.features) {
  ...
  const name =
    p.Concelho || p.concelho || p.CONCELHO || p.NAME_2 || p.name || p.Municipio ||
    p.municipio || p.MUNICIPIO || p.des_simpli || p.Des_Simpli || 'sem nome'
  let simplified
  try {
  ...
  const roundCoords = (c) =>
    Array.isArray(c[0]) ? c.map(roundCoords) : [round(c[0]), round(c[1])]
