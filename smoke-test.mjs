...
for (const a of Object.values(AIRCRAFT)) {
  check(
    `aeronave ${a.id}: speedRange valido`,
  ...
  check(
    `aeronave ${a.id}: payloads existem no catalogo`,
...
check(
  'custom default: camara em intervalos validos',
...
check(
  'custom default: FOV LiDAR 5-360 graus',
  inRange(DEFAULT_CUSTOM_SENSOR.fov, 5, 360),
   ...
   current {aircraftId, payloadId} objects both land on valid pairs. */
{
  ...
  check(
    'migra M3E',
    sameSel(migrateDroneSelection('M3E'), { aircraftId: 'M3E', payloadId: 'M3E_WIDE' }),
  )
  check(
    'migra M4T',
    sameSel(migrateDroneSelection('M4T'), { aircraftId: 'M4T', payloadId: 'M4T_WIDE' }),
  )
  check(
    'migra M300RTK para P1',
    sameSel(migrateDroneSelection('M300RTK'), { aircraftId: 'M300RTK', payloadId: 'P1' }),
  )
  check(
    'migra CUSTOM',
    sameSel(migrateDroneSelection('CUSTOM'), { aircraftId: 'CUSTOM', payloadId: 'CUSTOM' }),
  )
  check(
    'droneId desconhecido cai no default',
    sameSel(migrateDroneSelection('MAVIC_PRO'), DEFAULT_SELECTION),
  )
  check(
    'par valido passa intacto',
    sameSel(migrateDroneSelection({ aircraftId: 'M300RTK', payloadId: 'CUSTOM' }), {
      aircraftId: 'M300RTK',
      payloadId: 'CUSTOM',
    }),
  )
  check(
    'payload incompativel encaixa no primeiro da aeronave',
    sameSel(migrateDroneSelection({ aircraftId: 'M3E', payloadId: 'P1' }), {
      aircraftId: 'M3E',
      payloadId: 'M3E_WIDE',
    }),
  )
  check(
    'aeronave desconhecida cai no default',
    sameSel(migrateDroneSelection({ aircraftId: 'M600', payloadId: 'P1' }), DEFAULT_SELECTION),
  )
  check(
    'selecao ausente cai no default',
    sameSel(migrateDroneSelection(undefined), DEFAULT_SELECTION),
  )
  check(
    'default e valido',
...
check(
  'GSD: -60 = nadir x 1/sin60 (x1.1547)',
  Math.abs(computeGSD(sensor, 100, -60) / gsd - 1 / Math.sin(Math.PI / 3)) < 1e-9,
  computeGSD(sensor, 100, -60).toFixed(4),
)
check('GSD: -45 = nadir x 1.4142', Math.abs(computeGSD(sensor, 100, -45) / gsd - Math.SQRT2) < 1e-9)
check(
  'GSD: |pitch| < 20 -> null (quase-horizonte)',
   ...
   modelo antigo para os perfis que não mudaram. */
{
  ...
  check(
    'payload M3E = sensor de referencia',
    sM3E.type === 'camera' &&
      ['sensorWidth', 'sensorHeight', 'focalLength', 'imageWidth'].every(
        (k) => sM3E[k] === sensor[k],
      ),
  )
  const fpP1 = computeFootprint(resolveSensor(PAYLOADS.P1, DEFAULT_CUSTOM_SENSOR), 100)
  check(
    'P1 footprint across ~102.6 m',
    Math.abs(fpP1.across - 102.57) < 0.1,
    fpP1.across.toFixed(2),
  )
  ...
  check(
    'payload CUSTOM camara le o customSensor',
    sCustomCam.type === 'camera' && sCustomCam.sensorWidth === DEFAULT_CUSTOM_SENSOR.sensorWidth,
  )
  const sCustomLidar = resolveSensor(PAYLOADS.CUSTOM, {
    ...DEFAULT_CUSTOM_SENSOR,
    mode: 'lidar',
    fov: 75,
  })
  check(
    'payload CUSTOM lidar le o FOV editado',
    sCustomLidar.type === 'lidar' && sCustomLidar.fov === 75,
  )
  // lidar payload shape (arrives with T1.2): flies with the working FOV
  const sLidar = resolveSensor(
    { type: 'lidar', fov: 70.4, effectiveFov: 60 },
    DEFAULT_CUSTOM_SENSOR,
  )
...
/* 1c. YellowScan Mapper+ no M300 (T1.2) */
{
  ...
  check(
    'Mapper+ e payload lidar montavel no M300',
  ...
  check(
    'Mapper+ faixa ~141 m @100 m (FOV 70.4)',
    Math.abs(swath.across - 141.1) < 0.5,
    swath.across.toFixed(2),
  )
  const cut = computeFootprint(
    resolveSensor({ ...mp, effectiveFov: 60 }, DEFAULT_CUSTOM_SENSOR),
    100,
  )
  check(
    'Mapper+ corte 60 graus: faixa ~115.5 m',
    Math.abs(cut.across - 115.47) < 0.5,
  ...
  check(
    ...
    migrateDroneSelection({ aircraftId: 'M300RTK', payloadId: 'MAPPER_PLUS' }).payloadId ===
      'MAPPER_PLUS',
  )
}
...
/* 1d. Teto operacional AGL por payload (T1.3) */
{
  ...
  check(
    'teto AGL: 120 m avisa',
    w != null && w.cap === 100 && w.worstAgl === 120,
    w && `${w.worstAgl} > ${w.cap}`,
  )
  const wt = aglCapWarning(mp, 98, { terrainFollowActive: true, toleranceM: 5 })
  check(
    'teto AGL com terrain follow: 98+5 m avisa',
    wt != null && wt.worstAgl === 103,
...
{
  const swath = computeFootprint(
    resolveSensor(PAYLOADS.MAPPER_PLUS, DEFAULT_CUSTOM_SENSOR),
    100,
  ).across
  const d = lidarPointDensity({ prr: PAYLOADS.MAPPER_PLUS.maxPrr, speed: 10, swathM: swath })
  check(
    'densidade Mapper+ ~170 pts/m2 @100 m, 10 m/s',
    Math.abs(d.single - 170) < 2,
    d.single.toFixed(1),
  )
  check(
    'densidade na sobreposicao = 2x',
    Math.abs(d.overlap - 2 * d.single) < 1e-9,
    d.overlap.toFixed(1),
  )
  check(
    'velocidade mais baixa aumenta a densidade',
    lidarPointDensity({ prr: 240000, speed: 5, swathM: swath }).single > d.single,
  )
  check(
    'densidade sem dados validos -> null',
...
{
  check(
    'groupApplies: grupo universal aparece sempre',
    groupApplies({ titulo: 'x' }, 'camera') && groupApplies({ titulo: 'x' }, 'lidar'),
  )
  check(
    'groupApplies: grupo lidar so com lidar',
    groupApplies({ appliesTo: 'lidar' }, 'lidar') &&
      !groupApplies({ appliesTo: 'lidar' }, 'camera'),
  )
  check(
    'groupApplies: grupo face so com missao de fachada ativa',
  ...
  check(
    'groupApplies: face independente do tipo de sensor',
    groupApplies({ appliesTo: 'face' }, 'lidar', { face: true }),
  )
  check(
    'groupApplies: grupo corredor so com missao de corredor activa',
  ...
  check(
    'groupApplies: corredor e fachada nao se activam um ao outro',
...
/* 1e. Bateria por combinacao aeronave+payload (T1.4) */
{
  ...
  check(
    'bateria: override da combinacao vale',
    batteryMinFor(AIRCRAFT.M300RTK, 'MAPPER_PLUS', { 'M300RTK:MAPPER_PLUS': 38 }) === 38,
  )
  check(
    'bateria: outra combinacao nao e afetada',
    batteryMinFor(AIRCRAFT.M300RTK, 'P1', { 'M300RTK:MAPPER_PLUS': 38 }) === 55,
  )
  check(
    'bateria: override invalido cai no default',
    batteryMinFor(AIRCRAFT.M3E, 'M3E_WIDE', { 'M3E:M3E_WIDE': -5 }) === 45,
  )
  check(
    'bateria: overrides ausentes usam defaults',
    batteryMinFor(AIRCRAFT.M4T, 'M4T_WIDE') === 49 &&
      batteryMinFor(AIRCRAFT.CUSTOM, 'CUSTOM') === 25,
  )
}
...
check(
  'LiDAR swath ~140 m @FOV70/100m',
  Math.abs(lidar.across - 140.04) < 0.1,
  lidar.across.toFixed(2),
)
...
check(
  'bowtie inválido',
  vBad.valid === false && vBad.kinks.length > 0,
  `${vBad.kinks.length} kinks`,
)
...
if (plan90 && !plan90.error) {
  ...
  const diff = Math.abs(
    ((turf.bearing(plan90.lines[0][0], plan90.lines[0][1]) - b1 + 540) % 360) - 180,
  )
...
if (plan90 && !plan90.error) {
  ...
  rowKeys.forEach((k, r) => {
    ...
    if (r % 2 === 0) segs.forEach((s) => expected.push(s[0], s[1]))
      ...
      segs
        .slice()
        .reverse()
        .forEach((s) => expected.push(s[1], s[0]))
  })
  const identical =
  ...
  check(
    'R2.5: retangulo convexo = serpentina simples (1e-9 deg)',
    identical,
...
/* 5b. Overshoot por faixa (T2.2) */
{
  ...
  if (po && !po.error) {
    ...
    check(
      'overshoot: mesmo numero de faixas',
      po.stats.lineCount === plan90.stats.lineCount,
      `${po.stats.lineCount} vs ${plan90.stats.lineCount}`,
    )
    check(
      'overshoot: fotos sobre a area < fotos totais',
      po.stats.photoCountArea != null && po.stats.photoCountArea < po.stats.photoCount,
      `${po.stats.photoCountArea} < ${po.stats.photoCount}`,
    )
    check(
      'overshoot: fotos/area = fotos do plano sem overshoot',
    ...
    check(
      'overshoot conta para o percurso',
...
/* 5c. Fiada de amarracao perpendicular (T2.3) */
{
  ...
  check(
    'tie: +1 faixa',
    pt2 && !pt2.error && pt2.stats.lineCount === plan90.stats.lineCount + 1,
    pt2?.stats.lineCount,
  )
  ...
  check(
    'tie: atravessa a largura do bloco (~300 m)',
    Math.abs(lenTie - 300) < 10,
    lenTie.toFixed(1),
  )
  const midT = turf.midpoint(turf.point(tie[0]), turf.point(tie[1]))
  check(
    'tie: a meio e dentro da area',
  ...
  check(
    'crosshatch: fiada so na 1.a grelha',
    cx && !cx.error && cx.stats.lineCount >= 21 && cx.stats.lineCount <= 22,
    cx?.stats.lineCount,
  )
}
...
if (plan0 && !plan0.error) {
  check(
    'plan0 ~12-13 linhas (500/42.5)',
    plan0.stats.lineCount >= 12 && plan0.stats.lineCount <= 13,
    plan0.stats.lineCount,
  )
...
if (plan20 && !plan20.error) {
  check(
    'buffer aumenta área',
    plan20.stats.bufferedAreaHa > plan20.stats.areaHa * 1.15,
...
if (planL && !planL.error) {
  ...
  check(
    'planL linhas longas e curtas',
    lens.some((l) => l > 550) && lens.some((l) => l < 300),
...
/* 8a2. Decomposição celular boustrophedon (T3.1, Choset & Pignon 1998) */
{
  ...
  check(
    'decompose: braços adjacentes à base',
  ...
  check(
    'decompose: linha vazia separa células',
    gap.cells.length === 2 && gap.adjacency[0].size === 0,
  )
}
...
/* 8a3. Percurso côncavo-seguro num U (T3.1) */
{
  ...
  check(
    'U: percurso ≤ serpentina antiga',
    pu.stats.pathLengthM <= legacyLen + 1,
    `${pu.stats.pathLengthM.toFixed(0)} vs ${legacyLen.toFixed(0)} m`,
  )
  check(
    'U: ganho real sobre a serpentina',
    pu.stats.pathLengthM < legacyLen - 100,
...
/* 8a4. Direcao otima de voo (T3.2) */
{
  ...
  check(
    'otima: concorda com a diagonal do trapezio (±6°)',
    dT <= 6 && gen(trap, optT) <= gen(trap, edgeT),
  ...
  check(
    'otima: bate a aresta mais longa no U',
    nOptU < nEdgeU,
...
if (plan90 && !plan90.error) {
  ...
  check(
    'blocos por área: 4 blocos de 2 faixas',
    bArea?.length === 4 && bArea.every((b) => b.lines.length === 2),
    bArea?.map((b) => b.lines.length).join('+'),
  )
  check(
    'blocos numerados 1..n',
    bArea?.every((b, i) => b.id === i + 1),
  )
  ...
  check(
    'blocos por bateria: 3 blocos',
    bBat?.length === 3,
    bBat?.map((b) => b.lines.length).join('+'),
  )
  check(
    'tempo por bloco ≤ 210 s',
    bBat?.every((b) => b.timeS <= 215),
    bBat?.map((b) => Math.round(b.timeS)).join(','),
  )
  ...
  check(
    'trânsito à base contabilizado',
    bBase && bBase.length >= bBat.length && bBase[0].transitS > 100,
...
/* 8c-bis. Fits-check exclui a ligacao entre faixas (T0.5) */
{
  ...
  check(
    'fits sem ligacao: 1 bloco de 2 faixas',
    bb?.length === 1 && bb[0].lines.length === 2,
...
check(
  'grelha contorno ~750×500 m',
  Math.abs(outW - 750) < 8 && Math.abs(outH - 500) < 8,
  `${outW.toFixed(0)}×${outH.toFixed(0)}`,
)
const cellSides = grid.cells.map((c) => turf.distance(c[0], c[1], { units: 'meters' }))
check(
  'células ~250 m de lado',
  cellSides.every((s) => Math.abs(s - 250) < 5),
)
const centroids = grid.cells.map(
  (c) => turf.centroid(turf.polygon([[...c, c[0]]])).geometry.coordinates,
)
let snakeOk = true
...
check(
  'células em serpentina (vizinhas a ~250 m)',
  snakeOk,
  centroids
    .slice(1)
    .map((c, i) => turf.distance(centroids[i], c, { units: 'meters' }).toFixed(0))
    .join(','),
)
...
check(
  'plano por célula gerado (~6-7 faixas)',
  cellPlan && !cellPlan.error && cellPlan.stats.lineCount >= 6 && cellPlan.stats.lineCount <= 7,
...
check(
  'mosaico gerado',
  Array.isArray(mosaic) && mosaic.length >= 6 && mosaic.length <= 9,
  mosaic?.length,
)
...
check(
  'mosaico rodado 45° gerado',
  Array.isArray(mosaicRot) && mosaicRot.length > 0,
  mosaicRot?.length,
)
...
/* 8f. Alinhamento global: faixas oblíquas colineares entre células */
{
  ...
  if (pA?.lines && pB?.lines) {
    ...
    check(
      'faixas oblíquas em múltiplos do espaçamento (colineares)',
      worst < 1,
   ...
   (y 60-90 e 210-240 m) devolviam null e desapareciam da missao. */
{
  ...
  check(
    'A1: 10/10 celulas estreitas com plano (zero nulls)',
    plans.every((p) => p && !p.error),
    plans.map((p) => (p && !p.error ? '1' : '0')).join(''),
  )
  ...
  const composed = composeCellPlans(grid10.outline, [plans[0], null, plans[2]], {
    photoIntervalM: 0,
  })
  check(
    'A1: celula sem plano -> cell-uncovered [2]',
    composed?.error === 'cell-uncovered' && composed.cells.join(',') === '2',
  )
  check(
    'A1: varias celulas em falta listadas',
    composeCellPlans(grid10.outline, [null, plans[1], null], {}).cells.join(',') === '1,3',
  )
  check(
    'A1: erro de uma celula propaga-se',
    composeCellPlans(grid10.outline, [plans[0], { error: 'too-many-lines' }], {}).error ===
      'too-many-lines',
  )
  const full = composeCellPlans(grid10.outline, plans, { photoIntervalM: 0 })
  check(
    'A1: composicao completa soma as 10 celulas',
    full &&
      !full.error &&
      full.cellPlans.length === 10 &&
   ...
   vez (recuada 0.1 m para dentro da celula de cima), nunca duas */
{
  ...
  const pl = grid2b.cells.map((c) =>
    generateFlightLines(c, {
      spacingM: sp,
      angleDeg: 0,
      bufferPct: 0,
      photoIntervalM: iv,
  ...
  const lons = pl
    .flatMap((p) => p.lines.map((s) => ((s[0][0] + s[1][0]) / 2 - center[0]) * mLon))
    .sort((a, b) => a - b)
  const gaps = lons.slice(1).map((x, i) => x - lons[i])
  check(
    'A1: linha da aresta partilhada voada uma so vez (gap minimo ~espacamento)',
    Math.min(...gaps) > sp - 0.5,
    Math.min(...gaps).toFixed(2) + ' m',
  )
  check(
    'A1: a linha da aresta fica a 0.1 m da aresta, dentro de uma das celulas',
    lons.some((x) => Math.abs(x) > 0.05 && Math.abs(x) < 0.2),
    lons.map((x) => x.toFixed(2)).join(','),
  )
}
...
/* 8g. Lado do quadrado por bateria */
{
  ...
  check(
    'tempo do bloco ≤ tempo útil',
    timeS <= 25 * 60 * 0.7 + 1,
    `${Math.round(timeS)} s ≤ 1050 s`,
  )
...
/* 8h. Dupla grelha (crosshatch) */
{
  ...
  if (cross && !cross.error) {
    check(
      'crosshatch: 8 + 12-13 faixas',
      cross.stats.lineCount >= 20 && cross.stats.lineCount <= 21,
      cross.stats.lineCount,
    )
    const bearings = cross.lines.map((s) =>
      Math.round(((turf.bearing(s[0], s[1]) % 180) + 180) % 180),
    )
    ...
    check(
      'crosshatch: tempo ~2× grelha simples',
...
/* 8h2. Passagem nadir extra no preset 3D (R2.10) */
{
  ...
  check(
    'nadir: 3.a grelha = repetir a 1.a no fim',
    three && !three.error && three.stats.lineCount === two.stats.lineCount + plan90.stats.lineCount,
    `${three?.stats.lineCount} = ${two.stats.lineCount} + ${plan90.stats.lineCount}`,
  )
  check(
    'nadir: nadirStartLine e waypoint no fim das duas grelhas',
    three.nadirStartLine === two.stats.lineCount &&
      three.nadirStartWaypoint === two.stats.waypointCount,
  )
  check(
    'nadir: fotos somam a 3.a passagem',
    three.stats.photoCount === two.stats.photoCount + plan90.stats.photoCount,
  )
  check(
    'nadir: tempo cresce ~50% face ao crosshatch',
  ...
  check(
    'nadir: sem a opcao o crosshatch fica igual',
  ...
  check(
    'nadir: anotacao por bloco [3,3,2]@4 -> [null,1,0]',
    JSON.stringify(nadirLineLocalPerBlock([3, 3, 2], 4)) === '[null,1,0]',
  )
  check(
    'nadir: sem nadir -> tudo null',
    nadirLineLocalPerBlock([3, 3], null).every((v) => v === null),
  )
  check(
    'nadir: nadir na fronteira do bloco -> [null,0]',
  ...
  check(
    'nadir: WPML com -60 global e -90 no arranque da 3.a grelha',
    wlN.includes('<wpml:gimbalPitchRotateAngle>-60</wpml:gimbalPitchRotateAngle>') &&
      ...
      wlN.includes(
        `<wpml:actionGroupStartIndex>${three.nadirStartWaypoint}</wpml:actionGroupStartIndex>`,
      ),
  )
}
...
/* 8h3. Degradacoes silenciosas convertidas em erro explicito (A2) */
{
  ...
  check(
    'A2: segunda grelha nula -> crosshatch-failed, nunca um plano menor',
    r2?.error === 'crosshatch-failed' && !r2.lines,
  )
  calls = 0
  const nullThird = (ring, o) => (++calls === 3 ? null : generateFlightLines(ring, o))
  check(
    'A2: passagem nadir nula -> nadir-failed',
    generateFlightPlan(rectNS, { ...base, includeNadir: true }, nullThird)?.error ===
      'nadir-failed',
  )
  calls = 0
  const errSecond = (ring, o) =>
    ++calls === 2 ? { error: 'too-many-lines' } : generateFlightLines(ring, o)
  check(
    'A2: erro da segunda grelha propaga-se tal e qual',
    generateFlightPlan(rectNS, base, errSecond)?.error === 'too-many-lines',
  )
  check(
    'A2: sem stub o crosshatch continua a gerar normalmente',
...
/* 8i. Planeamento de GCPs */
{
  ...
  check(
    'GCPs todos dentro',
    g7.every((g) => turf.booleanPointInPolygon(turf.point(g.point), polyR)),
  )
  ...
  check(
    'GCPs em polígono côncavo dentro',
    gL.length >= 6 && gL.every((g) => turf.booleanPointInPolygon(turf.point(g.point), polyL2)),
    gL.length,
  )
}
...
/* 8j. Terrain follow (elevação sintética) */
{
  ...
  if (plan90 && !plan90.error) {
    ...
    check(
      'rampa linear → 2 wp/faixa',
      tf.perLine.every((n) => n === 2),
      tf.perLine.join(','),
    )
    ...
    check(
      'alturas seguem a rampa (~50 a ~150)',
      Math.abs(hMin - 50) < 3 && Math.abs(hMax - 150) < 3,
      `${hMin.toFixed(1)}–${hMax.toFixed(1)}`,
    )
    check(
      'elevMin/Max do terreno coerentes',
      tf.elevMin >= 145 && tf.elevMax <= 255,
     ...
     N-S passava a 17,8 m do solo com 100 m de AGL pedidos. */
  {
    ...
    const linhas = [
      [at(0, 0), at(1000, 0)],
      [at(1000, 1000), at(0, 1000)],
    ]
    const tf = terrainFollowLines(colina, linhas, {
      agl: 100,
      refElev: 200,
      toleranceM: 5,
      stepM: 40,
    ...
    check(
      'TF: ligacao entre linhas ganha waypoints sobre a colina',
      tf.perLink?.length === 2 && tf.perLink[0] === 0 && tf.perLink[1] >= 1,
      (tf.perLink ?? ['sem perLink']).join(','),
    )
    check(
      'TF: perLine continua a somar os waypoints, ligacoes incluidas',
    ...
    for (let i = 1; i < tf.waypoints.length; i++) {
      ...
      for (let s = 0; s <= 20; s++) {
        ...
        folgaMin = Math.min(
          folgaMin,
          z - colina.elevationAt(lo0 + (lo1 - lo0) * t, la0 + (la1 - la0) * t),
        )
      }
    }
    check(
      'TF: a rota nunca desce abaixo de AGL - tolerancia, ligacao incluida',
      folgaMin >= 100 - 5 - 0.5,
      `folga minima ${folgaMin.toFixed(1)} m`,
    )
    // sem relevo, uma ligacao curta nao ganha pontos espurios
    const plano = { elevationAt: () => 200 }
    const tfPlano = terrainFollowLines(
      plano,
      [
        [at(0, 0), at(500, 0)],
        [at(500, 40), at(0, 40)],
      ],
      { agl: 100, refElev: 200, toleranceM: 5 },
    )
    check(
      'TF: ligacao curta em terreno plano nao acrescenta waypoints',
...
/* 8j2. Plano medio da encosta (T4.5) */
{
  ...
  check(
    'encosta: inclinacao ~11.3 graus',
    fit && Math.abs(fit.slopeDeg - 11.31) < 0.3,
    fit?.slopeDeg?.toFixed(2),
  )
  check(
    'encosta: desce para Oeste (~270)',
    Math.abs(fit.downhillAzimuthDeg - 270) < 3,
    fit?.downhillAzimuthDeg?.toFixed(1),
  )
  check(
    'encosta: curvas de nivel N-S (~0)',
    fit.contourAzimuthDeg < 3 || fit.contourAzimuthDeg > 177,
    fit?.contourAzimuthDeg?.toFixed(1),
  )
  ...
  check(
    'encosta N: inclinacao ~19.3 graus',
    fitN && Math.abs(fitN.slopeDeg - 19.29) < 0.4,
    fitN?.slopeDeg?.toFixed(2),
  )
  check(
    'encosta N: desce para Sul e curvas E-O',
...
/* 8m. Agregado do projecto (E3.2) */
{
  ...
  check(
    'agregado: 2 planos validos, tempo e fotos somados',
...
check(
  'template.kml: templateType waypoint',
  tpl.includes('<wpml:templateType>waypoint</wpml:templateType>'),
)
check('template.kml: 16 placemarks', (tpl.match(/<Placemark>/g) || []).length === 16)

const wl = buildWaylinesWPML(wpmlParams)
check(
  'waylines: multipleDistance',
  wl.includes('multipleDistance') &&
    wl.includes(`<wpml:actionTriggerParam>${iv.toFixed(1)}</wpml:actionTriggerParam>`),
)
check('waylines: takePhoto', wl.includes('takePhoto'))
check(
  'waylines: gimbal nadir',
  wl.includes('<wpml:gimbalPitchRotateAngle>-90</wpml:gimbalPitchRotateAngle>'),
)
check('waylines: 16 índices', wl.includes('<wpml:index>15</wpml:index>'))
check(
  'waylines: actionGroupEndIndex 15',
  wl.includes('<wpml:actionGroupEndIndex>15</wpml:actionGroupEndIndex>'),
)

const wlTime = buildWaylinesWPML({ ...wpmlParams, triggerMode: 'time' })
check(
  'waylines tempo: multipleTiming ~2.1 s',
  wlTime.includes('multipleTiming') &&
    wlTime.includes('<wpml:actionTriggerParam>2.1</wpml:actionTriggerParam>'),
)

check(
  'waylines camara: grupos 0 (gimbal) e 1 (foto)',
...
check(
  'waylines sem intervalo: sem takePhoto, gimbal mantido',
...
check(
  'waylines Mapper+: sem acoes de camara',
...
/* 9b. Modo fachada (T4.2 — gerador puro) */
{
  ...
  check(
    'fachada: passo vertical ~6.7 m (<= 8.0)',
    Math.abs(fpl.stats.vStepM - 6.67) < 0.1 && fpl.stats.vStepM <= 8.0,
    fpl.stats.vStepM?.toFixed(2),
  )
  check(
    'fachada: alturas de 13.3 a 46.7 m',
    Math.abs(fpl.stats.heights[0] - 13.32) < 0.1 && Math.abs(fpl.stats.heights[5] - 46.68) < 0.1,
    `${fpl.stats.heights[0].toFixed(1)}-${fpl.stats.heights[5].toFixed(1)}`,
  )
  check(
    'fachada: espacamento na passagem ~10.5 m (<= 10.7)',
    Math.abs(fpl.stats.hStepM - 10.53) < 0.2 && fpl.stats.hStepM <= 10.7,
    fpl.stats.hStepM?.toFixed(2),
  )
  check(
    'fachada: GSD a 25 m ~0.67 cm/px',
    Math.abs(fpl.stats.gsdCm - 0.6715) < 0.005,
    fpl.stats.gsdCm?.toFixed(4),
  )
  check(
    'fachada: waypoints = passagens x pontos e 1 foto por waypoint',
  ...
  check(
    'fachada: rumo perpendicular a face (~180)',
  ...
  check(
    'fachada: viragem em altura no mesmo ponto',
    fpl.waypoints[n - 1][0] === fpl.waypoints[n][0] &&
      fpl.waypoints[n - 1][1] === fpl.waypoints[n][1] &&
  ...
  check(
    'fachada L: rumo 180 no inicio e 90 no fim',
  ...
  const facing = fpL.waypoints.map((w, i) => {
    ...
    const hits = turf.lineIntersect(
      turf.lineString([[w[0], w[1]], far.geometry.coordinates]),
      lLine,
    ).features
    if (hits.length === 0) return null
    return Math.min(
      ...hits.map((f) => turf.distance([w[0], w[1]], f.geometry.coordinates, { units: 'meters' })),
    )
  })
  const okFacing = facing.filter((d) => d != null && d > 22 && d < 28).length
  check(
    'fachada L: distancia a face segundo o rumo ~25 m',
  ...
  check(
    'fachada L: afastamento minimo nunca colapsa (>18 m)',
  ...
  const wlFace = buildWaylinesWPML({
    ...
    triggerMode: 'distance',
    sensorType: 'camera',
    perWaypoint: fpl.perWaypoint,
  })
  check(
    'fachada: WPML com rumo fixo em todos os waypoints',
    (wlFace.match(/smoothTransition/g) || []).length === fpl.waypoints.length,
  )
  check(
    'fachada: WPML com uma foto por waypoint',
...
/* 9b1. Piso de seguranca da fachada (C) */
{
  ...
  const mkFloor = (standoffM, extra = {}) =>
    generateFacePlan(wall, {
      sensor,
      faceHeightM: 30,
      standoffM,
      side: 'left',
  ...
  check(
    'C fachada: alturas estritamente crescentes com afastamento curto',
  ...
  check(
    'C fachada: sem piso activo a distribuicao mantem-se',
  ...
  check(
    'C fachada: piso acima do topo -> uma passagem no piso',
...
/* 9b2. Folga da fachada contra DSM local (R2.8) */
{
  ...
  check(
    'folga vertical: solo elevado apanha as passagens baixas',
...
/* 9b3. Persistencia da configuracao de fachada (E1.1) */
{
  ...
  check(
    'P1: deslocamento reduz a metade quando a velocidade duplica',
  ...
  const wlSpeed = buildWaylinesWPML({
    name: 'face',
    waypoints: s4.waypoints,
    perWaypoint: s4.perWaypoint,
    altitude: 47,
    speed: 4,
    wpml: wpmlParams.wpml,
    photoIntervalM: 0,
  ...
  check(
    'P1: autoFlightSpeed = velocidade configurada',
  ...
  check(
    'ticks: comprimento e direccao correctos',
...
/* 9c. Orbitas multi-nivel (T5.1 — gerador puro) */
{
  ...
  check(
    'orbita: raio 100 m com erro < 1 m',
  ...
  check(
    'orbita: gimbal trigonometrico por nivel',
    orb.perLevel[0].gimbalPitch === -11 &&
      orb.perLevel[1].gimbalPitch === -22 &&
  ...
  const wlOrb = buildWaylinesWPML({
    ...
    triggerMode: 'distance',
    sensorType: 'camera',
    perWaypoint: orb.perWaypoint,
    turnMode: orb.turnMode,
  })
  check(
    'orbita: WPML em voo curvo continuo',
    (wlOrb.match(/toPointAndPassWithContinuityCurvature/g) || []).length ===
      orb.stats.waypointCount && !wlOrb.includes('toPointAndStopWithDiscontinuityCurvature'),
  )
  check(
    'orbita: WPML com rumo e foto por waypoint',
  ...
  check(
    'orbita: turnMode por defeito inalterado',
  ...
  {
    const straightLineOf = (xml) => [
      ...new Set([...xml.matchAll(/<wpml:useStraightLine>([^<]*)</g)].map((m) => m[1])),
    ]
    const dampingOf = (xml) => [
      ...new Set([...xml.matchAll(/<wpml:waypointTurnDampingDist>([^<]*)</g)].map((m) => m[1])),
    ]

    check(
      'viragem: grelha/fachada mantém recta com paragem (inalterado)',
      straightLineOf(wlOrbBase).join() === '1' && dampingOf(wlOrbBase).join() === '0',
    )
    check(
      'viragem: órbita em curva contínua usa useStraightLine=0',
      straightLineOf(wlOrb).join() === '0',
      straightLineOf(wlOrb).join(),
    )
    check(
      'viragem: órbita não escreve amortecimento fora do intervalo',
      dampingOf(wlOrb).every((d) => Number(d) === 0),
      dampingOf(wlOrb).join(),
    )
    check(
      'viragem: coordinateTurn escreve amortecimento > 0 (obrigatório)',
      turnParams('coordinateTurn', 0).damping > 0 &&
        turnParams('toPointAndPassWithContinuityCurvature', 0).straightLine === 0,
    )
    check(
      'viragem: template.kml segue o mesmo modo da rota',
      buildTemplateKML({
        ...wpmlParams,
     ...
     (waylines-wpml.md) e as acções de segurança têm enums fechados. */
  {
    ...
    check(
      'missionConfig: globalRTHHeight presente (obrigatório)',
      tag(wlOrbBase, 'globalRTHHeight') != null,
      tag(wlOrbBase, 'globalRTHHeight'),
    )
    check(
      'missionConfig: RTH acima do tecto da missão',
       ...
       derivado só dela punha o regresso abaixo da própria rota. */
    {
      ...
      check(
        'R: RTH acima do waypoint mais alto, nao so da altitude nominal',
        rth > maisAlto,
        `${rth} m vs waypoint mais alto ${maisAlto} m`,
      )
      check(
        'R: rota com alturas proprias nao fica acima do RTH',
      ...
      const wlPlano = buildWaylinesWPML({
        ...wpmlParams,
        waypoints: planalto.map(([x, y]) => [x, y]),
        altitude: 100,
      })
      check(
        'R: sem alturas por waypoint, o RTH continua a sair da altitude',
      ...
      check(
        'R: RTH derivado limitado ao mesmo tecto que a validacao impoe',
    ...
    check(
      'missionConfig: acções de segurança configuráveis',
    ...
    check(
      'missionConfig: enum inválido cai no valor por omissão',
      tag(bogus, 'finishAction') === 'goHome' &&
        tag(bogus, 'executeRCLostAction') === 'goBack' &&
        tag(bogus, 'exitOnRCLost') === 'executeLostAction',
    )
    check(
      'missionConfig: enums declarados batem com a especificação',
  ...
  check(
    'orbita: levels por contagem/passo',
    generateOrbitPlan(center, {
      sensor,
      radiusM: 50,
  ...
  check(
    'orbitConfig: limites, arredondamento e lixo -> default',
    onorm.radiusM === 500 &&
      onorm.levelCount === 12 &&
      onorm.levelStartM === DEFAULT_ORBIT_CONFIG.levelStartM &&
      onorm.clockwise === false &&
      onorm.poi[0] === -8,
  )
...
{
  check('orbitConfig: speedMS default em projectos antigos', normalizeOrbitConfig({}).speedMS === 5)
  check(
    'orbitConfig: speedMS limitada a 1-10',
  ...
  check(
    'orbita: tempo reduz a metade quando a velocidade duplica',
  ...
  const wlOrbSpeed = buildWaylinesWPML({
    name: 'orbita',
    waypoints: o4.waypoints,
    perWaypoint: o4.perWaypoint,
    turnMode: o4.turnMode,
    altitude: 50,
    speed: 4,
    wpml: wpmlParams.wpml,
    photoIntervalM: 0,
  ...
  check(
    'orbita: autoFlightSpeed = velocidade configurada',
...
/* 9d. Pontos de inspecao (R2.9 — parte pura) */
{
  ...
  check(
    'inspecao: ordem por vizinho mais proximo',
  ...
  check(
    'inspecao: sem base comeca no primeiro ponto',
  ...
  check(
    'inspecao: waypoints com altura arredondada',
    wps.length === 3 && wps[2][2] === 55.6,
    wps[2][2],
  )
  check(
    'inspecao: heading/pitch so quando definidos',
    pw[0].heading === 45 &&
      pw[0].gimbalPitch === -30 &&
      pw[1].heading === undefined &&
      pw[1].gimbalPitch === undefined,
  )
  check(
    'inspecao: photo=false nao dispara',
    pw[1].actions.length === 0 &&
      pw[0].actions.includes('takePhoto') &&
      pw[2].actions.includes('takePhoto'),
  )
  ...
  check(
    'inspecao: WPML com 1 rumo fixo e 2 fotos',
  ...
  check(
    'reorder: move para a frente e para tras',
    reorderList(['a', 'b', 'c', 'd'], 0, 2).join('') === 'bcad' &&
      reorderList(['a', 'b', 'c', 'd'], 3, 1).join('') === 'adbc',
  )
  const orig = ['a', 'b']
  check(
    'reorder: indices invalidos devolvem a lista original intacta',
    reorderList(orig, 0, 5) === orig &&
      reorderList(orig, 1, 1) === orig &&
...
/* 10b. Acoes por waypoint no exportador (T4.1) */
{
  ...
  check(
    'T4.1: heading smoothTransition apenas no wp1',
    wlPW.includes('<wpml:waypointHeadingAngle>45</wpml:waypointHeadingAngle>') &&
      (wlPW.match(/smoothTransition/g) || []).length === 1,
  )
  check(
    'T4.1: headingAngleEnable=1 so onde ha heading',
    (wlPW.match(/<wpml:waypointHeadingAngleEnable>1<\/wpml:waypointHeadingAngleEnable>/g) || [])
      .length === 1,
  )
  check(
    'T4.1: grupo do wp1 com gimbal -30',
  ...
  check(
    'T4.1: ids de grupos consecutivos',
  ...
  check(
    'T4.1: sem perWaypoint nao ha smoothTransition nem grupos extra',
   ...
   intervalo, fotos so no nucleo, overshoot sem foto, modo distancia intacto */
{
  ...
  check(
    '8n passPoints 90/20: passos de 18 m',
    steps.every((s) => Math.abs(s - 18) < 0.01),
  ...
  check(
    '8n passPoints: extremos exactos',
    pts[0][0] === pA[0] && pts[0][1] === pA[1] && pts[5][0] === pB[0] && pts[5][1] === pB[1],
  )
  check(
    '8n normalizeTriggerMode',
    normalizeTriggerMode('bogus') === 'distance' &&
      normalizeTriggerMode(undefined) === 'distance' &&
      normalizeTriggerMode('time') === 'time' &&
      normalizeTriggerMode('waypoint') === 'waypoint',
  )
  ...
  if (pDist && !pDist.error && pWp && !pWp.error) {
    ...
    check(
      '8n modo distancia: byte a byte igual com/sem photoMode',
      JSON.stringify(pDist) === JSON.stringify(pDist2),
    )
    check(
      '8n modo distancia: sem perLine/perWaypoint, 2 waypoints/faixa',
      !('perLine' in pDist) && !('perWaypoint' in pDist) && pDist.waypoints.length === 2 * n,
    )
    check(
      '8n linhas iguais nos dois modos',
      JSON.stringify(pWp.lines) === JSON.stringify(pDist.lines),
    )
    check(
      '8n faixas de ~90 m',
      pWp.lines.every((l) => Math.abs(dist(l[0], l[1]) - 90) < 0.5),
      n,
    )
    check(
      '8n perLine: 6 waypoints por faixa',
      pWp.perLine.length === n && pWp.perLine.every((k) => k === 6),
      pWp.perLine.join(','),
    )
    check(
      '8n waypointCount = 6 x faixas',
      pWp.stats.waypointCount === 6 * n && pWp.waypoints.length === 6 * n,
    )
    check(
      '8n photoCount = waypoints com foto',
      pWp.stats.photoCount === 6 * n && pWp.perWaypoint.filter(Boolean).length === 6 * n,
      pWp.stats.photoCount,
    )
    ...
    pWp.perLine.forEach((k, li) => {
      for (let j = 1; j < k; j++) {
        if (Math.abs(dist(pWp.waypoints[idx + j - 1], pWp.waypoints[idx + j]) - 18) > 0.05)
          okStep = false
      }
      for (let j = 0; j < k; j++) {
    ...
    check(
      '8n overshoot: faixas de ~120 m',
      pOv.lines.every((l) => Math.abs(dist(l[0], l[1]) - 120) < 0.5),
    )
    check(
      '8n overshoot: 8 waypoints por faixa',
      pOv.perLine.every((k) => k === 8),
      pOv.perLine.join(','),
    )
    check(
      '8n overshoot: sem fotos a mais',
      pOv.stats.photoCount === 6 * n && pOv.stats.photoCountArea === null,
      pOv.stats.photoCount,
    )
    let okOv = true
    let o = 0
    ...
    check(
      '8n fiada de amarracao densificada (25 m / 20 -> 3 pontos)',
      pTie && pTie.perLine.length === n + 1 && pTie.perLine[n] === 3,
      pTie?.perLine?.join(','),
    )

    // WPML: um grupo takePhoto (reachPoint) por waypoint com foto, sem gatilho
    const wlWp = buildWaylinesWPML({
      ...wpmlParams,
      waypoints: pWp.waypoints,
      perWaypoint: pWp.perWaypoint,
      photoIntervalM: 0,
      triggerMode: 'waypoint',
    })
    const nPhoto = (
      wlWp.match(/<wpml:actionActuatorFunc>takePhoto<\/wpml:actionActuatorFunc>/g) || []
    ).length
    check('8n WPML: um takePhoto por waypoint com foto', nPhoto === 6 * n, nPhoto)
    check(
      '8n WPML: sem gatilho por distancia/tempo',
      !wlWp.includes('multipleDistance') && !wlWp.includes('multipleTiming'),
    )
    const groups = (wlWp.match(/<wpml:actionGroup>/g) || []).length
    check('8n WPML: grupos = gimbal inicial + 1 por foto', groups === 1 + 6 * n, groups)
    const starts = [
      ...wlWp.matchAll(/<wpml:actionGroupStartIndex>(\d+)<\/wpml:actionGroupStartIndex>/g),
    ].map((m) => Number(m[1]))
    const ends = [
      ...wlWp.matchAll(/<wpml:actionGroupEndIndex>(\d+)<\/wpml:actionGroupEndIndex>/g),
    ].map((m) => Number(m[1]))
    check(
      '8n WPML: cada grupo de foto no proprio waypoint (reachPoint)',
      (wlWp.match(/reachPoint/g) || []).length === groups &&
        starts.every((s, i) => s === ends[i]) &&
        new Set(starts.slice(1)).size === 6 * n,
    )
    const ids = [...wlWp.matchAll(/<wpml:actionGroupId>(\d+)<\/wpml:actionGroupId>/g)].map((m) =>
      Number(m[1]),
    )
    check(
      '8n WPML: ids de grupo consecutivos',
      ids.every((id, i) => id === i),
    )
    const wlOv = buildWaylinesWPML({
      ...wpmlParams,
      waypoints: pOv.waypoints,
      perWaypoint: pOv.perWaypoint,
      photoIntervalM: 0,
      triggerMode: 'waypoint',
    })
    check(
      '8n WPML overshoot: 6 fotos/faixa, ultimo waypoint sem grupo',
      (wlOv.match(/<wpml:actionActuatorFunc>takePhoto<\/wpml:actionActuatorFunc>/g) || [])
        .length ===
        6 * n &&
        !wlOv.includes(
          '<wpml:actionGroupStartIndex>' +
            (pOv.waypoints.length - 1) +
            '</wpml:actionGroupStartIndex>',
        ),
    )
    ...
    if (cut) {
      ...
      check(
        '8n blocos: waypoints densificados fatiados sem perda',
        total === pBig.waypoints.length,
        total + '/' + pBig.waypoints.length,
      )
      check(
        '8n blocos: perLine por bloco = 6 por faixa',
        cut.every((b) => b.perLine.length === b.lines.length && b.perLine.every((k) => k === 6)),
      )
      check(
        '8n blocos: fotos por bloco = 6 x faixas',
        cut.every((b) => b.perWaypoint.filter(Boolean).length === 6 * b.lines.length),
      )
      let off = 0
      let okSlice = true
      cut.forEach((b) => {
        if (
          ...
          JSON.stringify(pBig.waypoints.slice(off, off + b.waypoints.length))
        )
          okSlice = false
        off += b.waypoints.length
      })
      check('8n blocos: fatias contiguas pela ordem de voo', okSlice)
      const cutD = splitIntoBlocks(generateFlightLines(big, base), {
        mode: 'area',
        maxAreaHa: 0.5,
        speed: 5,
        spacingM: 10,
      ...
      check(
        '8n blocos modo distancia: forma intacta (2 waypoints/faixa, sem perLine)',
        cutD.length === cut.length &&
          cutD.every((b) => b.waypoints.length === 2 * b.lines.length && !('perLine' in b)),
      )
    }

    // dupla grelha + nadir e celulas: perWaypoint concatenado com offsets
    const pX = generateFlightPlan(rectB, {
      ...base,
      photoMode: 'waypoint',
      crosshatch: true,
      includeNadir: true,
    ...
    check(
      '8n dupla grelha + nadir: perLine/perWaypoint concatenados',
      pX &&
        !pX.error &&
        pX.perLine.length === pX.lines.length &&
        pX.perWaypoint.filter(Boolean).length === pX.stats.photoCount &&
        pX.stats.photoCount === pX.perLine.reduce((s, k) => s + k, 0),
    )
    check(
      '8n dupla grelha + nadir: nadirStartWaypoint coerente com perLine',
      pX &&
        pX.perLine.slice(0, pX.nadirStartLine).reduce((s, k) => s + k, 0) ===
          pX.nadirStartWaypoint &&
        pX.perWaypoint[pX.nadirStartWaypoint] != null,
    )
    const cB2 = rectangleFromAnchor(
      turf.destination(center, 0.03, 0, { units: 'kilometers' }).geometry.coordinates,
      90,
      25,
      90,
    )
    const plans = [rectB, cB2].map((r) =>
      generateFlightLines(r, { ...base, photoMode: 'waypoint' }),
    )
    const comp = composeCellPlans(rectB, plans, {
      photoIntervalM: 20,
      overshootM: 0,
      photoMode: 'waypoint',
    })
    check(
      '8n celulas: perWaypoint com offset da 1.a celula',
      comp &&
        comp.perWaypoint.filter(Boolean).length ===
          plans[0].stats.photoCount + plans[1].stats.photoCount &&
        comp.perWaypoint[plans[0].waypoints.length] != null &&
        comp.perLine.length === comp.lines.length,
    )
    check(
      '8n celulas: photoCountArea nulo no modo waypoint',
      comp &&
        comp.stats.photoCountArea === null &&
        comp.stats.photoCount === plans[0].stats.photoCount + plans[1].stats.photoCount,
    )
    const compD = composeCellPlans(
      rectB,
      [rectB, cB2].map((r) => generateFlightLines(r, { ...base, overshootM: 10 })),
      { photoIntervalM: 20, overshootM: 10 },
    )
    check(
      '8n celulas modo distancia: sem perLine, photoCountArea mantido',
      compD && !('perLine' in compD) && compD.stats.photoCountArea != null,
    )
  }
}
   ...
   referencia versionados. */
{
  ...
  const facePlanG = generateFacePlan(
    [
      [-9.14, 38.7],
      [-9.138, 38.7],
    ],
  ...
  const orbitPlanG = generateOrbitPlan([-9.14, 38.7], {
    sensor: sensorCam,
    radiusM: 60,
    levels: { count: 2, startM: 30, stepM: 20 },
    poiHeightM: 15,
    speed: 3,
  })
  ...
  const scenarios = [
    ...
    [
      ...
      {
        name: 'ref_face',
        waypoints: facePlanG.waypoints,
        perWaypoint: facePlanG.perWaypoint,
        altitude: 40,
        speed: 3,
        wpml: enums,
        photoIntervalM: 0,
    ...
    [
      ...
      {
        name: 'ref_orbit',
        waypoints: orbitPlanG.waypoints,
        perWaypoint: orbitPlanG.perWaypoint,
        turnMode: orbitPlanG.turnMode,
        altitude: 50,
        speed: 3,
        wpml: enums,
        photoIntervalM: 0,
  ...
  for (const [label, params] of scenarios) {
    ...
    check(
      `XML: ${label} template.kml analisa`,
      xmlWellFormed(tplX) === null,
      xmlWellFormed(tplX) ?? '',
    )
    check(
      `XML: ${label} waylines.wpml analisa`,
      xmlWellFormed(wlX) === null,
      xmlWellFormed(wlX) ?? '',
    )
  ...
  rejects(
    'longitude fora de alcance',
    { ...okParams, waypoints: [[999, 38.7]] },
    'waypoint-out-of-range',
  )
  rejects(
    'altura Infinity',
    { ...okParams, waypoints: [[-9.1, 38.7, Infinity]] },
    'height-not-finite',
  )
  ...
  check('limite de waypoints alinhado com wpml:index [0, 65535]', MAX_WAYPOINTS_PER_ROUTE === 65536)
  ...
  rejects(
    'intervalo de disparo negativo',
    { ...okParams, photoIntervalM: -5 },
    'param-out-of-range',
  )
  ...
  check(
    'velocidade e altitude plausiveis continuam a passar',
    Boolean(buildWaylinesWPML({ ...okParams, speed: 23, altitude: 120, rthHeightM: 150 })),
  )
  check(
    'validateExportParams devolve os parametros validos',
  ...
  check(
    'escapeXml preserva tab/LF/CR e escapa metacaracteres',
    escapeXml('a\tb\nc&d<e') === 'a\tb\nc&amp;d&lt;e',
  )
  ...
  check(
    'escapeXml remove substitutos isolados',
    !/[\uD800-\uDFFF]/.test(escapeXml(`a${hi}b${lo}c`)),
    JSON.stringify(escapeXml(`a${hi}b${lo}c`)),
  )
  check(
    'escapeXml preserva pares substitutos validos (emoji)',
     ...
     Gerador congruencial com semente fixa para qualquer falha ser reproduzivel. */
  {
    ...
    check(
      'fuzz: 60 planos aleatorios produzem XML analisavel',
      bad === 0 && ran > 40,
     ...
     importa em Node, por isso lê-se a fonte. */
  {
    ...
    const codes = [
      ...new Set([...src.matchAll(/new MissionExportError\(\s*'([^']+)'/g)].map((m) => m[1])),
    ]
    const missing = codes.filter((c) => !dict.includes(`'export.err.${c}'`))
    check(
      'i18n: todos os códigos de erro de exportação têm tradução',
     ...
     silencio. Regenerar com UPDATE_GOLDEN=1 node smoke-test.mjs. */
  {
    ...
    for (const [label, params] of scenarios) {
      ...
      for (const [suffix, xml] of [
        ...
        let ref = null
        try {
          ref = readFileSync(file, 'utf8')
        } catch {
          ref = null
        }
        check(
          `referencia: ${label}.${suffix}`,
          ref === xml,
     ...
     FORMADO — um KMZ que o comando nem chega a abrir. */
  {
    ...
    const recusa = (extra) => {
      try {
        buildWaylinesWPML({ ...baseVal, ...extra })
        return false
      } catch {
        return true
      }
    ...
    check(
      'fronteira: takeOffSecurityHeightM fora de (0, 200] recusado',
      recusa({ takeOffSecurityHeightM: -50 }) &&
        recusa({ takeOffSecurityHeightM: 0 }) &&
        recusa({ takeOffSecurityHeightM: 5000 }) &&
        !recusa({ takeOffSecurityHeightM: 30 }),
    )
    check(
      'fronteira: pitch por waypoint fora de [-120, 60] recusado',
      recusa({ perWaypoint: [{ gimbalPitch: 85 }] }) &&
        recusa({ perWaypoint: [{ gimbalPitch: -300 }] }) &&
        !recusa({ perWaypoint: [{ gimbalPitch: 60 }, { gimbalPitch: -120 }] }),
    )
    check(
      'fronteira: rumo por waypoint fora de [-180, 360) recusado',
      recusa({ perWaypoint: [{ heading: 400 }] }) &&
        recusa({ perWaypoint: [{ heading: -200 }] }) &&
     ...
     com rumos fora do intervalo, falha aqui em vez de passar despercebido. */
  {
    ...
    for (const nome of nomes) {
      ...
      try {
        texto = readFileSync(
          new URL(`./tests/golden/${nome}.waylines.wpml`, import.meta.url),
          'utf8',
        )
      } catch {
        continue
    ...
    check(
      'A2: todos os rumos dos ficheiros de referencia em [-180, 180]',
      total > 0 && fora === 0,
      `${fora} fora de ${total} rumos`,
    )
  }
}
   ...
   ou a sobreposicao frontal falha na berma exterior. */
{
  ...
  for (let a = 0; a <= 180; a += 10) {
    ...
    halfCircle.push([
      -9.14 + rLon * Math.cos((a * Math.PI) / 180),
      38.7 + (60 / 110574) * Math.sin((a * Math.PI) / 180),
    ])
  }
  ...
  /* 12a. Distribuicao das passagens. */
  {
    ...
    const spacing = 42.23
    check(
      'corredor: pegada cobre o corredor -> passagem central unica',
      passOffsets(50, spacing, across).join() === '0',
    )
    const o = passOffsets(150, spacing, across)
    check(
      'corredor: desvios simetricos e centrados em 0',
      o.length > 1 && Math.abs(o[0] + o[o.length - 1]) < 1e-9,
      o.map((v) => v.toFixed(1)).join(','),
    )
    const gaps = o.slice(1).map((v, i) => v - o[i])
    check(
      'corredor: espacamento entre passagens nunca excede o exigido',
      gaps.every((g) => g <= spacing + 1e-9),
      `${Math.max(...gaps).toFixed(2)} <= ${spacing}`,
    )
    check('corredor: passagem exterior cobre a berma', Math.max(...o) + across / 2 >= 150)
    ...
    check(
      'corredor: desvios cobrem sempre a largura pedida, sem recorte',
     ...
     cobertura na curva. */
  {
    ...
    const mid = out[1]
    check(
      'corredor: esquadria a 90 graus fica a 20 m dos dois segmentos',
    ...
    check(
      'corredor: inversao aguda cai em bisel finito',
  ...
  {
    const plan = generateCorridorPlan(halfCircle, {
      ...baseOpts,
      altitude: 60,
      bufferM: 120,
      photoIntervalM: 15,
    ...
    check(
      'corredor: curva apertada produz plano valido',
      Boolean(plan?.lines?.length),
      plan?.error ?? '',
    )
    const axisM = halfCircle.map(([lo, la]) => [
      (lo + 9.14) * (111320 * Math.cos((38.7 * Math.PI) / 180)),
      (la - 38.7) * 110574,
    ])
    let violations = 0
    for (const seg of plan.lines) {
      ...
      const own = plan.stats.offsets
        .map(Math.abs)
        .reduce((best, o) =>
          Math.abs(o - Math.min(...ds)) < Math.abs(best - Math.min(...ds)) ? o : best,
        )
      for (const d of ds) if (d < own - Math.max(0.5, own * 0.02)) violations += 1
    }
    check(
      'corredor: nenhum waypoint cai dentro de uma dobra',
      violations === 0,
      `${violations} violacoes`,
    )
    check(
      'corredor: passagens impossiveis sao eliminadas, nao voadas em laco',
     ...
     havia faixa por voar. */
    {
      ...
      check(
        'C2 corredor: eixo recto nao perde nem parte passagens',
        recto.stats.droppedPasses === 0 &&
          recto.stats.splitPasses === 0 &&
    ...
    for (const [rotulo, eixo] of eixos) {
      ...
      for (const seg of plan.lines) {
        ...
        for (let i = 0; i < m.length; i++) {
          // o ponto em si
          let d = Infinity
          for (let k = 1; k < eixoM.length; k++)
            d = Math.min(d, distSeg(m[i], eixoM[k - 1], eixoM[k]))
          ...
          for (let t = 1; t < passos; t++) {
            ...
            let dq = Infinity
            for (let k = 1; k < eixoM.length; k++)
              dq = Math.min(dq, distSeg(q, eixoM[k - 1], eixoM[k]))
            pior = Math.max(pior, dq)
          }
        }
      }
      check(
        `corredor cobertura: ${rotulo} nunca sai da meia-largura pedida`,
  ...
  for (const [label, line] of [
    ...
    check(
      `corredor: eixo ${label} -> um troco por passagem`,
      plan.stats.runCount === plan.stats.passCount,
      `${plan.stats.runCount}/${plan.stats.passCount}`,
    )
  }
  ...
  /* 12e. Amostragem por comprimento de arco de cada passagem. */
  {
    ...
    check(
      'corredor: fotos consecutivas nunca acima do intervalo',
      worst <= 20 * 1.02,
      `${worst.toFixed(2)} m para 20 m`,
    )
    ...
    check(
      'corredor: cada passagem amostrada pelo seu proprio arco',
      new Set(counts).size > 1,
      counts.join(','),
    )
    check(
      'corredor: cada waypoint leva accao de fotografia',
      plan.perWaypoint.filter(Boolean).length === plan.waypoints.length,
    )
    check(
      'corredor: perLine soma aos waypoints',
     ...
     erro, copiando a regra do modo fachada, onde a camara e mesmo exigida. */
  {
    const lidar = resolveSensor(
      { id: 'X', type: 'lidar', fov: 70, effectiveFov: 70 },
      DEFAULT_CUSTOM_SENSOR,
    )
    ...
    check(
      'corredor: payload LiDAR gera plano valido',
      !plan.error && plan.stats.passCount > 1,
      plan.error ?? `${plan.stats.passCount} passagens`,
    )
    check(
      'corredor: LiDAR espaca pelas passagens da largura de varrimento',
    ...
    check(
      'corredor: WPML LiDAR sem accoes de camara nem de gimbal',
  ...
  /* 12f. Modo distancia: sem accoes por waypoint, geometria preservada. */
  {
    ...
    check(
      'corredor: modo distancia nao emite accoes por waypoint',
      plan.perWaypoint === undefined && plan.perLine === undefined,
    )
    check('corredor: modo distancia estima fotos pelo comprimento', plan.stats.photoCount > 0)
    ...
    check(
      'corredor: simplificacao preserva os extremos exactos de cada passagem',
      plan.lines.length === dense.lines.length &&
        plan.lines.every((l, i) => {
          const d = dense.lines[i]
          return (
            turf.distance(l[0], d[0], { units: 'meters' }) < 0.01 &&
            turf.distance(l[l.length - 1], d[d.length - 1], { units: 'meters' }) < 0.01
          )
        }),
    )
    check(
      'corredor: simplificacao reduz mesmo o numero de pontos',
      plan.lines.reduce((n, l) => n + l.length, 0) < dense.lines.reduce((n, l) => n + l.length, 0),
    )
  }
  ...
  {
    check(
      'corredor: resample mantem todos os vertices originais',
      [
        [0, 0],
        [50, 0],
        [50, 40],
        ...
        resamplePolyline(
          ...
          7,
        ).some((q) => Math.hypot(q[0] - v[0], q[1] - v[1]) < 1e-9),
      ),
    )
    const arc = sampleByArcLength(
      [
        [0, 0],
        [100, 0],
        [100, 100],
    ...
    check(
      'corredor: amostragem por arco fixa os extremos',
      arc[0][0] === 0 &&
        arc[0][1] === 0 &&
        arc[arc.length - 1][0] === 100 &&
        arc[arc.length - 1][1] === 100,
    )
    check(
      'corredor: amostragem por arco respeita o passo',
      arc.slice(1).every((q, i) => Math.hypot(q[0] - arc[i][0], q[1] - arc[i][1]) <= 30 + 1e-6),
    )
    check(
      'corredor: simplificacao remove colineares e guarda o canto',
      simplifyPolyline(
        [
          [0, 0],
          [10, 0],
    ...
    check(
      'corredor: comprimento de polilinha',
      Math.abs(
        polylineLength([
          [0, 0],
          [3, 4],
    ...
    check(
      'corredor: trava marca o corte em vez de coser o resto',
      huge.truncated === true && huge[huge.length - 1][0] < 300000,
      `truncated=${huge.truncated}, ultimo x=${huge[huge.length - 1][0]}`,
    )
    check(
      'corredor: eixo longo demais e recusado, nao entregue com um salto',
      generateCorridorPlan([toLL(0, 0), toLL(22000, 0), toLL(22000, 4000)], {
        ...baseOpts,
        altitude: 30,
        bufferM: 60,
        sideOverlapPct: 90,
    ...
    check(
      'corredor: eixo normal continua a gerar plano',
       ...
       passagens a acabar quilometros antes do fim. */
    {
      ...
      check(
        'corredor: eixo dos meandros cabe na amostragem',
        resamplePolyline(rioM, passo).truncated !== true,
      )
      const cortado = offsetRuns(rioM, 60, passo)
      check(
        'corredor: desvio cortado pela trava sobe marcado e vazio',
      ...
      const rio = generateCorridorPlan(rioLL, {
        ...baseOpts,
        altitude: 30,
        bufferM: 60,
        sideOverlapPct: 90,
      ...
      check(
        'corredor: desvio longo demais e recusado mesmo com o eixo dentro da trava',
      ...
      check(
        'corredor: os mesmos meandros com passo largo continuam a dar plano',
        Boolean(rioLargo?.lines?.length),
        rioLargo?.error ?? '',
      )
    }
       ...
       do eixo — 5 passagens rendiam 9 trocos e o painel acusava 4 partidas. */
    {
      ...
      check(
        'corredor: vertice repetido nao gera junta',
        dup.length === 3 &&
          dup.every((q) => Math.abs(pointPolylineDistance(q, eixoRecto) - 30) < 1e-6),
        `${dup.length} pontos`,
      )
      const plano = generateCorridorPlan(
        [toLL(0, 0), toLL(500, 0), toLL(500, 0), toLL(1000, 0)],
        baseOpts,
      )
      check(
        'corredor: eixo recto com vertice repetido nao parte passagens',
        !plano.error &&
          plano.stats.splitPasses === 0 &&
          plano.stats.runCount === plano.stats.passCount,
        plano.error ?? `${plano.stats.passCount} passagens, ${plano.stats.runCount} trocos`,
      )
      const norm = normalizeCorridorConfig({
        centreline: [
          [-9.14, 38.7],
          [-9.14, 38.7],
          [-9.13, 38.7],
    ...
    check(
      'corredor: desvio nulo devolve o proprio eixo',
      offsetRuns(
        [
          [0, 0],
          [10, 0],
    ...
    const ringBuf = corridorBufferRing(
      [
        [-9.14, 38.7],
        [-9.13, 38.7],
      ],
    ...
    check(
      'corredor: anel ilustrativo fecha e envolve o eixo',
      Array.isArray(ringBuf) &&
        ringBuf.length >= 4 &&
        turf.booleanPointInPolygon(turf.point([-9.135, 38.7]), turf.polygon([ringBuf])),
      `${ringBuf?.length ?? 0} vertices`,
    )
    check(
      'corredor: anel invalido devolve null, nao rebenta',
  ...
  /* 12h. Entrada degenerada devolve erro controlado, nunca excepcao. */
  {
    ...
    for (const [label, line, over, want] of cases) {
      let got
      try {
        got = generateCorridorPlan(line, { ...baseOpts, ...over })
      } catch (e) {
        got = { threw: e.message }
      }
  ...
  /* 12i. Persistencia da configuracao (projectos antigos, lixo). */
  {
    ...
    check(
      'corredor: config em falta cai nos valores por omissao',
      d.bufferM === DEFAULT_CORRIDOR_CONFIG.bufferM &&
        d.centreline === null &&
        d.photoMode === 'distance',
    )
    const junk = normalizeCorridorConfig({
      bufferM: 'x',
      speedMS: 999,
      photoMode: 'zz',
      centreline: [
    ...
    check(
      'corredor: config invalida e saneada, nao rebenta',
      junk.bufferM === DEFAULT_CORRIDOR_CONFIG.bufferM &&
        junk.speedMS <= 25 &&
        junk.photoMode === 'distance' &&
        junk.centreline === null,
   ...
   area. Um grupo por intervalo contiguo; a ligacao longa fica de fora. */
{
  ...
  check(
    'disparo: ligacao curta mantem o intervalo, longa quebra-o',
    JSON.stringify(r1) === '[[0,3],[4,5]]',
    JSON.stringify(r1),
  )
  ...
  check(
    'disparo: pontos da ligacao (terrain follow) ficam fora do intervalo',
    JSON.stringify(r2) === '[[0,3],[7,8]]',
    JSON.stringify(r2),
  )
  check(
    'disparo: sem limite, um so intervalo',
  ...
  const grupos = [
    ...xml.matchAll(
      /<wpml:actionGroupStartIndex>(\d+)<\/wpml:actionGroupStartIndex>\s*<wpml:actionGroupEndIndex>(\d+)</g,
    ),
  ].map((m) => [Number(m[1]), Number(m[2])])
  check(
    'disparo: um actionGroup multipleDistance por intervalo, no waypoint onde comeca',
  ...
  check(
    'disparo: ids dos grupos consecutivos (gimbal 0, disparos 1 e 2)',
  ...
  check(
    'disparo: sem intervalos, um grupo a cobrir a rota inteira',
    (buildWaylinesWPML(base).match(/multipleDistance/g) ?? []).length === 1 &&
      buildWaylinesWPML(base).includes('<wpml:actionGroupEndIndex>8<'),
  )
  const recusaR = (triggerRanges) => {
    try {
      buildWaylinesWPML({ ...base, triggerRanges })
      return false
    } catch {
      return true
    }
  ...
  check(
    'disparo: intervalos tortos recusados na fronteira',
    recusaR([[0, 9]]) &&
      recusaR([[3, 1]]) &&
      recusaR([
        [0, 4],
   ...
   nove modulos; a variante Safe e a unica com semantica diferente */
{
  ...
  check(
    'U: metro por grau de longitude no equador = 111320',
    Math.abs(metersPerDegLon(0) - 111320) < 1e-9,
  )
  check(
    'U: encolhe com o cosseno da latitude (60 graus -> metade)',
    Math.abs(metersPerDegLon(60) - 111320 / 2) < 1e-6,
    metersPerDegLon(60).toFixed(2),
  )
  check(
    'U: simetrico no hemisferio sul',
    Math.abs(metersPerDegLon(-39.5) - metersPerDegLon(39.5)) < 1e-9,
  )
  check(
    'U: variante Safe tem piso de 1 m/grau (divisoes junto aos polos)',
    metersPerDegLonSafe(90) === 1 && metersPerDegLon(90) < 1e-6,
    `${metersPerDegLonSafe(90)} vs ${metersPerDegLon(90).toExponential(1)}`,
  )
  check(
    'U: fora dos polos a variante Safe e igual a normal',
