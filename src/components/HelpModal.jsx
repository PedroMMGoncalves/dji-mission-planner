  ...
  return (
    <>
      ...
      <ul className="list-none">
        ...
        <Li><strong>Corredor</strong> — para infraestruturas lineares (estradas, condutas, linhas de água, linhas eléctricas). Clique «Desenhar» e marque o <em>eixo</em> no mapa; conclua com duplo clique. Defina a <strong>meia-largura</strong> (distância coberta de cada lado do eixo — a largura total é o dobro) e o número de passagens paralelas sai daí, da altitude e da sobreposição lateral. Escolha o disparo por distância ou por waypoint. É <strong>apenas nadir</strong>: não suporta seguimento de terreno nem divisão por bateria, e as passagens voam a uma altitude única relativa ao ponto de descolagem.</Li>
      </ul>
      ...
      <ul className="list-none">
        ...
        <Li><strong>«Passagens partidas»</strong> (corredor) — onde o eixo curva mais apertado do que a distância da passagem ao eixo, a linha desviada dobrar-se-ia sobre si própria e o drone voaria um laço. A app parte a passagem em troços em vez de a voar dobrada, e diz quantos partiram. Nessas zonas a cobertura real é menor do que a faixa desenhada no mapa, que mostra sempre a largura <em>pedida</em>: alargue a curva do eixo ou reduza a meia-largura se precisar de cobertura contínua.</Li>
  ...
  return (
    <>
      ...
      <ul className="list-none">
        ...
        <Li><strong>Corridor</strong> — for linear infrastructure (roads, pipelines, watercourses, power lines). Click “Draw” and mark the <em>centreline</em> on the map; finish with a double click. Set the <strong>half-width</strong> (distance covered on each side of the centreline — the total width is twice that) and the number of parallel passes follows from it, the altitude and the side overlap. Choose distance or per-waypoint triggering. It is <strong>nadir only</strong>: no terrain following and no battery splitting, and the passes fly at a single altitude relative to the take-off point.</Li>
      </ul>
      ...
      <ul className="list-none">
        ...
        <Li><strong>“Split passes”</strong> (corridor) — where the centreline curves tighter than a pass's distance from it, the offset line would fold back on itself and the aircraft would fly a loop. The app splits the pass into runs instead of flying it folded, and reports how many split. Coverage there is narrower than the band drawn on the map, which always shows the <em>requested</em> width: ease the bend or reduce the half-width if you need continuous coverage.</Li>
