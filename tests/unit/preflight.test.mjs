...
import { hasBlockers, preflightArea, preflightCounts, preflightPlan, usableBatteryMin, WPML_MAX_WAYPOINTS } from '../../src/mission/preflight.js'
...
const base = () => ({
  plan, blocks: null, photoMode: 'distance', terrainFollow: { enabled: false, tolerance: 5 }, terrainCovers: false,
  terrainResult: null, basePoint: em(-50, -50), baseDistance: 70, speed: 8, batteryMin: 30, reservePct: 30,
  aglWarn: null, triggerWarn: null,
})
...
describe('preflightArea', () => {
  ...
  test('sem plano ou com erro: um bloqueio e mais nada', () => {
    ...
    expect(items).toEqual([{ level: 'block', code: 'plan-error', params: { error: 'too-many-lines' } }])
  })

  test('seguir terreno: foto por waypoint, relevo em falta e erro do cálculo bloqueiam', () => {
    const tf = { enabled: true, tolerance: 5 }
    expect(codes(preflightArea({ ...base(), terrainFollow: tf, photoMode: 'waypoint' }))).toContain('terrain-photo-waypoint')
    expect(codes(preflightArea({ ...base(), terrainFollow: tf, terrainCovers: false }))).toContain('terrain-not-loaded')
    const err = preflightArea({ ...base(), terrainFollow: tf, terrainCovers: true, terrainResult: { error: 'Ref fora' } })
    expect(err.find((i) => i.code === 'terrain-error').params.msg).toBe('Ref fora')
    // terreno bom: nenhum bloqueio
    const ok = preflightArea({ ...base(), terrainFollow: tf, terrainCovers: true, terrainResult: { waypoints: plan.waypoints, blocks3: null } })
  ...
  test('waypoints: aviso acima de 2000 numa rota, bloqueio acima do limite WPML; blocos contam o maior', () => {
    ...
    const huge = { ...plan, waypoints: Array.from({ length: WPML_MAX_WAYPOINTS + 1 }, () => em(0, 0)) }
    ...
    const blocks = [{ id: 1, waypoints: huge.waypoints.slice(0, 100), timeS: 60, transitS: 0 }, { id: 2, waypoints: huge.waypoints.slice(0, 100), timeS: 60, transitS: 0 }]
    expect(hasBlockers(preflightArea({ ...base(), plan: huge, blocks }))).toBe(false)
    // com terrain follow conta a rota densificada
    const tfItems = preflightArea({ ...base(), terrainFollow: { enabled: true, tolerance: 5 }, terrainCovers: true, terrainResult: { waypoints: many.waypoints, blocks3: null } })
  ...
  test('bateria: tempo mais trânsito acima do útil avisa; por bloco identifica o bloco', () => {
    ...
    const blocks = [{ id: 1, waypoints: [], timeS: 10 * 60, transitS: 30 }, { id: 2, waypoints: [], timeS: 22 * 60, transitS: 30 }]
    const bl = preflightArea({ ...base(), plan: longo, blocks }).filter((i) => i.code === 'battery-block')
  ...
  test('avisos passados (tecto AGL, obturador) e lembrete sem base', () => {
    const items = preflightArea({ ...base(), basePoint: null, baseDistance: null, aglWarn: { cap: 100, worstAgl: 105.4 }, triggerWarn: { actualS: 0.5, minS: 0.7, maxSpeed: 7.1 } })
  ...
  test('preflightPlan (outros modos): mesmas regras sem terreno nem base', () => {
    ...
    expect(codes(preflightPlan({ plan, batteryMin: 30, reservePct: 30 }))).toEqual(['heights-relative'])
    const longo = { ...plan, stats: { ...plan.stats, flightTimeS: 40 * 60 } }
    expect(codes(preflightPlan({ plan: longo, batteryMin: 30, reservePct: 30 }))).toContain('battery')
  })
  ...
  test('todos os códigos têm mensagem em PT e EN', () => {
    const all = new Set([
      ...
      ...codes(preflightArea({ ...base(), terrainFollow: { enabled: true }, photoMode: 'waypoint' })),
      ...codes(preflightArea({ ...base(), terrainFollow: { enabled: true } })),
      ...codes(preflightArea({ ...base(), terrainFollow: { enabled: true }, terrainCovers: true, terrainResult: { error: 'e' } })),
      ...codes(preflightArea({ ...base(), plan: { ...plan, waypoints: Array(WPML_MAX_WAYPOINTS + 1).fill(em(0, 0)) } })),
      ...codes(preflightArea({ ...base(), plan: { ...plan, waypoints: Array(2500).fill(em(0, 0)) } })),
      ...codes(preflightArea({ ...base(), basePoint: null, aglWarn: { cap: 1, worstAgl: 2 }, triggerWarn: { actualS: 1, minS: 1, maxSpeed: 1 } })),
      ...codes(preflightArea({ ...base(), plan: { ...plan, stats: { flightTimeS: 1e5 } } })),
      ...codes(preflightArea({ ...base(), plan: { ...plan, stats: { flightTimeS: 1e5 } }, blocks: [{ id: 1, waypoints: [], timeS: 1e5 }] })),
    ])
