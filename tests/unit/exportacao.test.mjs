...
import { buildTemplateKML, buildWaylinesWPML, validateExportParams } from '../../src/utils/exporters.js'

const JUNK = /(^|>)\s*(NaN|undefined|Infinity|null)\s*(<|$)/
const wpml = { droneEnumValue: 77, droneSubEnumValue: 0, payloadEnumValue: 66, payloadSubEnumValue: 0, payloadPositionIndex: 0 }
...
describe('buildWaylinesWPML', () => {
  test('parâmetros válidos dão XML bem formado, sem lixo, rumos em [-180, 180] e um Placemark por waypoint', () => {
    fc.assert(
      fc.property(params.chain((p) => perWaypointFor(p.waypoints.length).map((pw) => ({ ...p, perWaypoint: pw }))), (p) => {
  ...
  test('um grupo de disparo por intervalo, e só quando há disparo', () => {
    const intervalos = (n) =>
      fc
        ...
        .filter((r) => r.length > 0)
    fc.assert(
      fc.property(params.chain((p) => intervalos(p.waypoints.length).map((r) => ({ ...p, triggerRanges: r }))), (p) => {
        const xml = buildWaylinesWPML({ ...p, wpml })
...
describe('validateExportParams', () => {
  const base = { waypoints: [[-9.14, 38.7], [-9.13, 38.7]], altitude: 100, speed: 8, wpml, photoIntervalM: 20, triggerMode: 'distance', sensorType: 'camera' }
  const recusa = (extra) => expect(() => validateExportParams({ ...base, ...extra })).toThrow()

  test('fora do domínio é recusado, dentro é aceite', () => {
    fc.assert(
      fc.property(fc.double({ min: -1000, max: 0, noNaN: true }), (altitude) => { recusa({ altitude }) }),
      { numRuns: 50 },
    )
    fc.assert(
      fc.property(fc.double({ min: 30.0001, max: 1e6, noNaN: true }), (speed) => { recusa({ speed }) }),
      { numRuns: 50 },
    )
    fc.assert(
      fc.property(fc.double({ min: 360, max: 1e4, noNaN: true }), (heading) => { recusa({ perWaypoint: [{ heading }] }) }),
      { numRuns: 50 },
    )
    fc.assert(
      fc.property(fc.double({ min: 60.0001, max: 1e3, noNaN: true }), (gimbalPitch) => { recusa({ perWaypoint: [{ gimbalPitch }] }) }),
      { numRuns: 50 },
    )
    fc.assert(
      fc.property(fc.double({ min: 1, max: 300, noNaN: true }), fc.double({ min: 0.1, max: 30, noNaN: true }), (altitude, speed) => {
